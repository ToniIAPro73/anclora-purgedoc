...
if APP_ENV == "production":
    if not raw_cors or raw_cors.strip() == "*":
        origins = ["https://purgedoc.anclora.com"]
    else:
        origins = [o.strip() for o in raw_cors.split(",") if o.strip()]
        if "*" in origins:
            raise RuntimeError("CORS_ORIGINS cannot contain '*' with credentials in production")
else:
    if not raw_cors or raw_cors.strip() == "*":
        origins = ["http://localhost:3000", "http://127.0.0.1:3000", "http://localhost:8001", "http://127.0.0.1:8001"]
    else:
        origins = [o.strip() for o in raw_cors.split(",") if o.strip()]
...
async def create_session(
    ...
    session_store.create_session(session_id, current_user.id)
    lifecycle_manager.register_session(session_id)
    metadata_store.session_started(session_id, datetime.now(timezone.utc), user_id=current_user.id)
    return {"session_id": session_id, "status": "active"}

@api_router.get("/sessions/{session_id}/expiry")
async def get_session_expiry(session_id: str = Depends(require_owned_session)):
...
async def delete_session(session_id: str = Depends(require_owned_session)):
...
async def get_profiles(current_user: UserRow = Depends(get_current_user_required)):
...
async def validate_regex_pattern(payload: ValidateRegexPayload, current_user: UserRow = Depends(get_current_user_required)):
...
async def test_custom_rule(payload: TestRulePayload, current_user: UserRow = Depends(get_current_user_required)):
...
async def compute_ruleset_hash(ruleset: CustomRuleset, current_user: UserRow = Depends(get_current_user_required)):
...
async def upload_document(
    session_id: str = Depends(require_owned_session),
...
async def load_synthetic_fixture(
    fixture_name: str,
    session_id: str = Depends(require_owned_session),
    profile_id: Optional[str] = None,
    current_user: UserRow = Depends(get_current_user_required)
):
    if not ENABLE_DEV_FIXTURES:
        raise HTTPException(status_code=403, detail="Dev fixtures are disabled in this environment.")
    session_dir = session_store.get_session_dir(session_id)
    doc_id = str(uuid.uuid4())
...
async def analyze_document(doc_id: str = Depends(require_owned_document), payload: Optional[AnalyzeDocumentPayload] = None):
...
async def get_document_matches(doc_id: str = Depends(require_owned_document)):
    if doc_id not in session_store.documents:
        raise HTTPException(status_code=404, detail="Documento no encontrado.")
...
async def update_match_status(payload: MatchUpdatePayload, match_id: str = Depends(require_owned_match)):
    found_match = None
...
async def bulk_update_matches(payload: BulkMatchUpdatePayload, doc_id: str = Depends(require_owned_document)):
...
async def get_page_image(page_num: int, doc_id: str = Depends(require_owned_document)):
    if doc_id not in session_store.documents:
        raise HTTPException(status_code=404, detail="Documento no encontrado.")
...
async def purge_and_verify_document(doc_id: str = Depends(require_owned_document)):
    """
...
async def download_purged_file(doc_id: str = Depends(require_owned_document)):
    paths = session_store.doc_file_paths.get(doc_id, {})
...
async def download_audit_json(doc_id: str = Depends(require_owned_document)):
    paths = session_store.doc_file_paths.get(doc_id, {})
...
async def download_audit_pdf(doc_id: str = Depends(require_owned_document)):
    paths = session_store.doc_file_paths.get(doc_id, {})
...
async def export_encrypted_ruleset_endpoint(payload: ExportEncryptedRulesetPayload, current_user: UserRow = Depends(get_current_user_required)):
    try:
...
async def preview_rules_encrypted_endpoint(payload: PreviewEncryptedRulesetPayload, current_user: UserRow = Depends(get_current_user_required)):
    try:
...
async def get_batch_configuration(current_user: UserRow = Depends(get_current_user_required)):
    return get_batch_config()

@api_router.post("/sessions/{session_id}/batches")
async def create_batch(session_id: str = Depends(require_owned_session), payload: Optional[CreateBatchPayload] = None):
...
async def get_batch_details(batch_id: str = Depends(require_owned_batch)):
...
async def upload_documents_to_batch(
    ...
    batch_id: str = Depends(require_owned_batch)
):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def update_batch_document_profile(doc_id: str, payload: UpdateBatchDocumentProfilePayload, batch_id: str = Depends(require_owned_batch)):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def delete_batch_endpoint(batch_id: str = Depends(require_owned_batch)):
    batch = session_store.batches.get(batch_id)
    if not batch:
...
async def cancel_batch_document(doc_id: str, batch_id: str = Depends(require_owned_batch)):
    batch = session_store.batches.get(batch_id)
...
async def cancel_entire_batch(batch_id: str = Depends(require_owned_batch)):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def start_batch_analysis(batch_id: str = Depends(require_owned_batch), payload: Optional[StartBatchAnalysisPayload] = None):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def start_batch_purge(batch_id: str = Depends(require_owned_batch), payload: Optional[StartBatchPurgePayload] = None):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def download_batch_zip(batch_id: str = Depends(require_owned_batch)):
    batch = session_store.batches.get(batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Lote no encontrado.")
...
async def stream_batch_events(
    ...
    if not current_user:
        raise HTTPException(status_code=401, detail="Autenticación requerida para suscribirse a eventos.")
